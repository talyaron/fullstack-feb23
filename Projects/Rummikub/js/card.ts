...
function dealCardsToPlayers() {
    try {
        players.forEach(player => {
            ...
            const dealCardToPlayers = setInterval(() => {
                const indexCardToDeal = Math.floor(Math.random() * (pool.length - 1))
                const cardToDeal = pool[indexCardToDeal]
