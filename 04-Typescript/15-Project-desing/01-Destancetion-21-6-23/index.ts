...
class Flight {
    ...
    calculateDistanceInKm(): number | undefined {
        try {
            ...
            if (!lat1 || !lat2 || !lon1 || !lon2) throw new Error("Missing lat or lng")
...
function renderLoggedUser(passenger: Passanger, rootElement: HTMLElement | null) {
    try {
...
function handleCalculate(ev: any) {
    try {
        ...
        const airportFrom = airports.find(airport => airport.id === airportIdFrom);
        const airportTo = airports.find(airport => airport.id === airportIdTo);
        if (!airportTo || !airportFrom) throw new Error("Couldnt find one of the airport");
