...
var dishesArray = [new Dish("pasta", 42, "https://cdn.pixabay.com/photo/2014/04/22/02/55/pasta-329521_1280.jpg"), new Dish("sushi", 48, "https://cdn.pixabay.com/photo/2017/10/16/09/00/sushi-2856544_1280.jpg"), new Dish("salad-Japanese", 55, "https://cdn.pixabay.com/photo/2014/10/03/15/41/salad-471861_1280.jpg", 4), new Dish("ramen", 65, "https://cdn.pixabay.com/photo/2017/04/04/00/36/ramen-2199962_1280.jpg", 4), new Dish("bread", 22, "https://cdn.pixabay.com/photo/2019/05/06/14/24/bread-4183225_1280.jpg", 4)];
var emptyDishArray = [];

function addDishToOrder(dishName, dishNumber) {
  var newOrderDish = dishesArray.find(function (dish) {
    return dish.dishName == dishName;
  });
  if (newOrderDish) newOrderDish.dishNumber = dishNumber;
  return newOrderDish;
} //------------------------------------Table----------------------------------------
...
var TableOrderHandler =
/** @class */
function () {
  function TableOrderHandler(table, dishesOrdered) {
    if (dishesOrdered === void 0) {
      dishesOrdered = emptyDishArray;
    }
...
function addDiner(event) {
  ...
  tablesOrder.find(function (table) {
    return table.table.numOfTable == numOfTable;
  }).table.numOfDiners++;
}
...
function reduceDiner(event) {
  var numOfTable = event.target.parentNode.attributes.table.value;
  var numOfDiners = event.target.parentNode.querySelector(".numberOfDiner");
  if (numOfDiners.innerHTML == 0) return;else numOfDiners.innerHTML--;
  tablesOrder.find(function (table) {
    return table.table.numOfTable == numOfTable;
  }).table.numOfDiners--;
} //onclick table
...
function openMenu(event) {
  ...
  var chosenTable = tablesOrder.find(function (tbl) {
    return tbl.table.numOfTable == numChosenTable;
  });
...
function addDishToOrderList(event) {
  ...
  var chosenDish = dishesArray.find(function (dish) {
    return dish.dishName == dishName;
  });
