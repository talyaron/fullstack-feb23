...
const colors :string[]  = ['orange', 'white', 'black'];
...
// Print the updated array.
const fruits :string[] = ['strawberry', 'banana', 'kiwi', 'apple', 'orange'];
fruits.splice(2,1);
console.log(fruits);
...
const ages :number[] = [23, 45, 80, 4, 17];
...
for (let i = 0; i < 10; i++){
    const randomNumbers = (Math.floor(Math.random() * 200));
...
function getNumber (array :number[] = [1,5,7,4], element :number) {
    const index :number = array.findIndex(item => item === element);
...
function insertsDashes (evenNumbers:number) {
    const num = evenNumbers.toString();
    let result = '';
    for(let i = 0; i < num.length ; i++ ) {
        const inToNumber = num[i];
